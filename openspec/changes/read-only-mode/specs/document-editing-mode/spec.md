# Document editing mode — read-only lock

Requirement deltas for the collaborative document view
(`DocumentsController#show` with real-time text collaboration enabled).
...
applies even to users who are allowed to edit the document.
...
- **AND** the mode toggle shows **Reading** selected
...
collaborative document for users who are allowed to edit it. Users who are
not allowed to edit SHALL NOT be offered the toggle and SHALL continue to see
...
allowed to edit, and choosing **Reading** again SHALL return it to a
read-only presentation. Switching mode SHALL NOT drop the real-time
collaboration session or discard local undo history.
